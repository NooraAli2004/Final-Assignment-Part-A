...
class SocialMedia: #creating a class called social media
    def __init__(self): #initilizing the constructor function and 'self' being the instance of the class
        self.posts_by_datetime = {} #creating a dictionary tostore posts by their datetime (key)

        self.posts = [] #creating a list to store posts sorted by datetie so its easier to find them for a specific range

        self.view_counter = 0 #initiziizing a counter for the number of views on each post
    ...
    def add_post(self, datetime, post, posted_by, views=0): #defining a function for adding new posts to social media with 4 parameters and 'views' being optional
        ...
        heapq.heappush(self.posts_by_views, (-views, self.view_counter, datetime)) # Adding a max heap for retrieving post with most views, "-views" is negated to make the heap behave like a max heap where the maximum is at the roots
